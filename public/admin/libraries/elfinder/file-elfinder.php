...
if (empty($_SESSION['user']) || empty($_SESSION['imageManagement'])) {
    header('Location: /');
}
?>
<!DOCTYPE html>
            ...
            if(mode == 'chosefile'){
                $('#elfinder').elfinder({
                    ...
                    getFileCallback : function(file) {
                        $('#lbl_'+control, window.opener.document ).attr('src',file['url']);
                        $('#hd_'+control, window.opener.document ).val(file['url']);
                        window.close();
                    },
                    resizable: false
                }).elfinder('instance');
            }else if(mode == 'tninsert'){
                ...
                $('#elfinder').elfinder(
                    // 1st Arg - options
                    {
                        ...
                        // Connector URL
                        url : 'php/connector.php?mode=' + mode,
                        ...
                        getFileCallback : function(file) {
                            getMeta(
                                ...
                                function(width, height) {
                                    $($('.'+control, window.opener.document )[3]).val(width);
                                    $($('.'+control, window.opener.document )[4]).val(height);
                                }
                            );
                            $('.'+control, window.opener.document ).first().val(file['url']);
                            setTimeout(function () {
                                window.close();
                            }, 500)
